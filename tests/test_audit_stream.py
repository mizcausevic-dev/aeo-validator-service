...
from aeo_validator_service import audit_stream
...
class TestEmit:
    ...
    async def test_emit_is_noop_when_disabled(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        async with httpx.AsyncClient(transport=transport) as client:
            await audit_stream.emit(client, kind="watch_created", payload={"x": 1})
        assert captured == []
    ...
    async def test_emit_posts_to_events_endpoint_when_enabled(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        async with httpx.AsyncClient(transport=transport) as client:
            await audit_stream.emit(
                client,
                kind="watch_drifted",
                payload={"watch_id": "abc", "url": "https://x/"},
            )
        assert len(captured) == 1
        body = captured[0]
        assert body["kind"] == "watch_drifted"
        assert body["source"] == "aeo-validator-service"
        assert body["payload"]["watch_id"] == "abc"

    @pytest.mark.asyncio
    async def test_emit_swallows_server_error_silently(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("AUDIT_STREAM_URL", "http://audit.local/")
        ...
        transport = httpx.MockTransport(handler)
        async with httpx.AsyncClient(transport=transport) as client:
            await audit_stream.emit(client, kind="watch_drifted", payload={})
    ...
    async def test_emit_swallows_connection_refused(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        transport = httpx.MockTransport(handler)
        async with httpx.AsyncClient(transport=transport) as client:
            await audit_stream.emit(client, kind="watch_validity_flipped", payload={})
